<%--@elvariable id="discussion" type="com.jsun.site.entity.Discussion"--%>
<%--@elvariable id="replies" type="java.util.List<com.jsun.site.entity.Reply>"--%>
    ...
    <c:choose>
      <c:when test="${fn:length(replies) > 0}">
        <c:forEach items="${replies}" var="reply" varStatus="i">
          <b>Reply #${i.count}</b><i>  - <c:out value="${reply.user}" /> - <c:out value="${reply.created}" /></i><br />
          <c:out value="${reply.message}" /><br /><br/>
        </c:forEach>
      </c:when>
      <c:otherwise>
        <i>There are no replies yet.</i><br /><br />
      </c:otherwise>
    </c:choose>
    <b>Post Reply</b><br /><br />
