...
public class DefaultReplyService implements ReplyService {
	...
	public List<Reply> getRepliesForDiscussion(long discussionId) {
	  List<Reply> list = replyRepository.getForDiscussion(discussionId);
	...
	public void saveReply(Reply reply) {
	  ...
	  if(reply.getId() < 1) {
		...
		this.notificationService.sendNotification("Reply posted", "Someone posted to \"" + discussion.getSubject() + ".\"", recipients);
	  } else {
